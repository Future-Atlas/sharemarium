...
test('Stripe lookup failures block destructive account deletion', () => {
  assert.doesNotMatch(source, /lookupResponse\.status === 404[^]*return/)
  assert.match(source, /Stripe subscription lookup failed with HTTP/)
  assert.match(source, /Unable to cancel the active subscription before account deletion/)
...
test('Stripe subscription DELETE does not request proration or a final invoice', () => {
